...
func runLSTMTest(t *testing.T, model *ImplicitLSTMModel, train *Interactions,
...
func TestLSTMHinge(t *testing.T) {
	...
	runLSTMTest(t, model, &train, &test, expectedMRR)
}

func TestLSTMWARP(t *testing.T) {
	data, err := GetMovielens()
	if err != nil {
		panic(err)
	}
...
func TestLSTMDeallocation(t *testing.T) {
	numItems := 1000000
...
func runEWMATest(t *testing.T, model *ImplicitEWMAModel, train *Interactions,
	test *Interactions, expectedMrr float32) {
	...
	t.Log("Deserializing...")
	deserializedModel := &ImplicitEWMAModel{}
	err = deserializedModel.UnmarshalBinary(serialized)
	if err != nil {
		t.Errorf("Couldn't deserialize")
	}
	...
	// Make sure that using the model on the copy does not segfault, and
	// is handled correctly.
	var copy ImplicitEWMAModel = *model
	if copy.model == nil {
		t.Errorf("Copy model should be non-nil")
	}
...
func TestEWMAHinge(t *testing.T) {
	data, err := GetMovielens()
	if err != nil {
		panic(err)
	}
	...
	t.Log("Building model...")
	model := NewImplicitEWMAModel(data.NumItems())

	// Set the hyperparameters.
	model.MaxSequenceLength = 128
	model.ItemEmbeddingDim = 32
	model.LearningRate = 0.16
	model.L2Penalty = 0.0004
	model.NumEpochs = 10
...
func TestEWMADeallocation(t *testing.T) {
	numItems := 1000000
	data := NewInteractions(1, numItems)
	...
	for i := 0; i < 10; i++ {
		model := NewImplicitEWMAModel(numItems)
		_, err := model.Fit(&data)
		if err != nil {
			t.Errorf("Can't fit: %v", err)
		}
